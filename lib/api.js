...
const GD = function(params) {
  ...
  function createUser(data, cb) {
    const { login, email, password, verifyPassword, firstName, lastName, ssoProvider, language } = data;
  ...
  function deleteUser(data, cb) {
    request({
      url: `https://${options.hostName}/gdc/projects/${options.projectId}/users/${data.userId}`,
      port: options.port,
      method: 'DELETE',
      headers: {
        Cookie: GDCAuthTT,
